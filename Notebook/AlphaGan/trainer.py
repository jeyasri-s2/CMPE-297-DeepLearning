...
class Trainer(object):
    ...
    def train(self):
        ...
        for step in range(start, self.total_step):
            ...
            noise1 = torch.Tensor(real_images.size()).normal_(0, 0.01 * (step + 1 - self.total_step) / (step+1))

            noise2 = torch.Tensor(real_images.size()).normal_(0, 0.01 * (step +1 - self.total_step) / (step+1))
    ...
    def build_model(self):
        ...
        self.E = Encoder(self.batch_size, self.imsize, self.z_dim, self.d_conv_dim)
        self.D = Discriminator(self.batch_size,self.imsize, self.z_dim, self.d_conv_dim)
        if self.parallel:
